namespace DataFlow.Core.Blocks
{
    ...
    public abstract class InjectableTransformManyBlockAsync<TInput, TOutput> : IPropagatorBlock<TInput, TOutput>, ITargetBlock<TInput>, ISourceBlock<TOutput>
    {
        ...
        protected InjectableTransformManyBlockAsync()
        {
            transformBlock = new Lazy<TransformManyBlock<TInput, TOutput>>(() => new TransformManyBlock<TInput, TOutput>(TransformAsync));
        }

        protected InjectableTransformManyBlockAsync(ExecutionDataflowBlockOptions blockOptions)
        {
            transformBlock = blockOptions != null
                                        ? new Lazy<TransformManyBlock<TInput, TOutput>>(() => new TransformManyBlock<TInput, TOutput>(TransformAsync, blockOptions))
                                        : new Lazy<TransformManyBlock<TInput, TOutput>>(() => new TransformManyBlock<TInput, TOutput>(TransformAsync));
        }

        protected abstract Task<IEnumerable<TOutput>> TransformAsync(TInput message);
