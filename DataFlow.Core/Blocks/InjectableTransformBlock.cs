namespace DataFlow.Core.Blocks
{
    ...
    public abstract class InjectableTransformBlock<TInput, TOutput> : IPropagatorBlock<TInput, TOutput>, ITargetBlock<TInput>, ISourceBlock<TOutput>
    {
        ...
        protected InjectableTransformBlock()
        {
            transformBlock = new Lazy<TransformBlock<TInput, TOutput>>(() => new TransformBlock<TInput, TOutput>(Transform));
        }

        protected InjectableTransformBlock(ExecutionDataflowBlockOptions blockOptions)
        {
            transformBlock = blockOptions != null
                                        ? new Lazy<TransformBlock<TInput, TOutput>>(() => new TransformBlock<TInput, TOutput>(Transform, blockOptions))
                                        : new Lazy<TransformBlock<TInput, TOutput>>(() => new TransformBlock<TInput, TOutput>(Transform));
        }

        protected abstract TOutput Transform(TInput message);
